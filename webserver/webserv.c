...
int content_type(char* file, char* ct);
void send_error(FILE* fp);
void error_handling(char* message);
int isBinary(char* ct);
...
/* 입출력을 바이너리로 할지, 스트링으로 할지 결정해주는 함수*/
int isBinary(char* ct)
{    
	if(!strcmp(ct, "image/jpeg"))
    {
        return 1;
    } 
    else 
